...
def show_entries():
    ...
    entries = query_db('select * from customers where deleted=0 order by id asc')
...
def show_account(account_id):
    if not session.get('logged_in'):
        return redirect(url_for('login'))
    account = query_db('select * from customers where deleted=0 and id=?', (account_id,), one=True)
    if request.content_type == 'application/json':
        if not account:
        ...
        if not account:
            ...
            account['created_at'] = time.ctime(account['created_at']) + ' (UTC)'
            account['updated_at'] = time.ctime(account['updated_at']) + ' (UTC)'
...
def add_account():
    ...
    if request.method == 'GET':
        return render_template('add_account.html', account=account)
    else:
        timestamp = time.time()
        g.db.execute('''insert into customers
            (account_name, account_number, controller_ip, controller_id, admin_user, admin_pass, notes,
             created_at, updated_at, deleted)
            values (?, ?, ?, ?, ?, ?, ?, ?, ?, 0)''',
            (request.form['account_name'], request.form['account_number'], request.form['controller_ip'],
             request.form['controller_id'], request.form['admin_user'], request.form['admin_pass'],
    ...
    if request.method == 'GET':
        ...
        if request.content_type == 'application/json':
            ...
            g.db.execute('''insert or replace into customers
                (id, account_name, account_number, controller_ip, controller_id, admin_user, admin_pass, notes,
                 created_at, updated_at, deleted)
                values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)''',
                (account_id, request.form['account_name'], request.form['account_number'],
                 request.form['controller_ip'], request.form['controller_id'], request.form['admin_user'],
                 request.form['admin_pass'], request.form['notes'], account['created_at'], timestamp))
            g.db.commit()
